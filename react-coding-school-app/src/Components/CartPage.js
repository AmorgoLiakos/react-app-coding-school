  ...
  return (
    <Container>
      {CartItemsTotal.map((item, index) => (
        <CartItem key={item.id} url={item.imageURL} quantity={item.quantity} price={item.price} handleCartMinus={() => handleCartMinus(index)} handleCartPlus={() => handleCartPlus(index)} />
      ))}
    </Container>
  )
}
